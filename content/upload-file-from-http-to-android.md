---
title: http 中的上传文件以及 android 平台的实现
date: 2021-07-02T22:34:20+08:00
summary: 介绍软件如何实现上传文件的功能
---
...
通过分割符，我们可以在 body 中存放多个内容，可以是普通键值对数据。
如第三个参数；也可以是二进制文件，如前两个图片，它们是通过 content-type 来指定类型。
对于文件来说，我们需要指定 filename ，具体内容则是二进制文件。

需要注意的是，在 content-type 中指定的分割符，在实际内容中我们需要**在每个分割符开头以及 body 结尾多加上--**

## 客户端实现

上传文件在 retrofit 中的接口对应实现如下
...
请注意，这时我们的参数就都要加上 Part 的注解了，表示每个参数是 多部分的一部分。

对于参数为普通字符串的情况，我们指定为 RequestBody 类型即可。
而文件的话，就需要 MultipartBody.Part。关于Part 注解中的参数，如果类型是 MultipartBody.Part 内容将被忽略，而如果类型为 RequestBody 该值将直接与其内容类型一起使用。
...
对于文件类型， createFormData 需要我们传递一个 RequestBody 类型的对象，也就是上传文件的具体细节。其他参数中，第一个表示文件上传对应的接受者，即 http 请求中的 name；第二个参数为文件名称。

```kotlin
Part createFormData(String name, @Nullable String filename, RequestBody body)
```

我们使用 create 方法来创建，这里我们有两种方式。一种是通过输入流将文件读到 byte 数组，一种是直接传递文件。
...
至于普通字符串，则没什么可说的。我们需要将请求体的格式都指定为 multipart/form-data。
